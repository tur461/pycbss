...
def notifyDoctorAboutPatient(O, pid, dmid, typ):
        ...
        if len(getRedundantPatient(O, pid)) > 1:
                #print(f'len of getRedundantPatient() is <= 1, returning without notifying doctor about uid: {pid}')
                return
        t = O.lDoctorWSConnections
        for x in t:
        #        td[x.ID] = x.PidList
                if pid in x.PidList:
                        send(x, d)
        #print(f'Doctors {td} notified about uid: {pid}')
